...
class Package extends Distribution {
    public static readonly file = "package.json";
    public static readonly folder = "ci";

    private static readonly resolve = () => Path.join( Process.cwd(), Package.folder, Package.file );
    private static readonly source = () => Path.join( Process.cwd(), Package.file );
    private static readonly target = () => Package.source();

    public static readonly contents?: typeof import("../package.json") = JSON.parse( FS.readFileSync( Package.source(), { encoding: "utf-8" } ) );
