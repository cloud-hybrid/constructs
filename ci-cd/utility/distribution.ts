...
class Distribution {
    ...
    public readonly home: string = OS.homedir();

    /*** Current Module Directory */
    protected static readonly CWD = Path.dirname( import.meta.url.replace( "file" + ":" + "//", "" ) );
    /*** Current Module Working Directory */
    protected static readonly MWD = Path.dirname( Distribution.CWD );
    /*** Current Package's Working Directory */
    protected static readonly PWD = Path.dirname( Distribution.MWD );

    /*** Compatability Wrapper around Node.js's `require` */
    protected static readonly Import = Module.createRequire( Distribution.PWD );
    ...
    protected static write: (
        path: ( string | Buffer | URL | number ),
        data: ( string | Uint8Array | Uint8ClampedArray | Uint16Array | Uint32Array | Int8Array | Int16Array | Int32Array | BigUint64Array | BigInt64Array | Float32Array | Float64Array | DataView )
    ) => Promise<void> = Utility.promisify( FS.writeFile );
    ...
    protected static remove: ( path: FS.PathLike, options?: ( FS.RmOptions | undefined ) ) => Promise<void> = Utility.promisify( FS.rm );
    ...
    static async delete( path: ( string ), retries: number, force: boolean, recursive: boolean ) {
